    ...
    <section className="w-full py-10 px-4">
      <div className="mx-auto  max-w-[1280px] bg-white  overflow-hidden">
        ...
        <div className=" pt-6 sm:px-4 px-2 mx-auto">
          ...
          Whether you need to dominate local searches, rank globally, or appear in AI platforms like ChatGPT – 
I'll create a tailored strategy that gets you there. No outsourcing. No shortcuts. Just proven results.
          </p>
  ...
  <path d="M7.29448 11.025L3.17615 13.335L4.09781 8.70336L0.632812 5.50669L5.32281 4.94669L7.29448 0.665022L9.26615 4.94669L13.9561 5.50669L10.4911 8.70336L11.4128 13.335L7.29448 11.025Z" fill="#FF8E53"/>
</svg>
                ))}
</span>
        ...
        <div className="mt-6 mx-4 sm:mx-6  rounded-[14px] border border-[#E5EAF1] ">
         <div className="bg-colourful rounded-t-[14px] py-4 sm:py-6 md:py-8">
          ...
          <form onSubmit={handleSubmit} className="md:py-8 py-4 inter-font space-y-4 sm:px-5 px-3.5 md:px-[48px]">
                ...
                {[
                  ...
                  <label
                    ...
                    className={`flex  items-center gap-2 border sm:h-12 h-9 border-[#E5EAF1] rounded-[10px] px-3 py-2 bg-[#F9FAFB] text-sm cursor-pointer ${
                      errors.services && formData.services.length === 0 ? 'border-red-500' : ''
                    }`}
                  >
                    <Checkbox
            ...
            <div className="space-y-2">
              <Label className="text-sm font-medium">What is your primary goal?* <span className="text-red-500">*</span></Label>
              <Select
                value={formData.primaryGoal}
                onValueChange={(value) => updateFormData({ primaryGoal: value })}
              >
                <SelectTrigger className={`input w-full bg-[#F9FAFB] ${errors.primaryGoal ? 'border-red-500' : ''}`}>
                  <SelectValue placeholder="Select your goal" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="rank-higher">Rank higher on Google</SelectItem>
                  <SelectItem value="local-customers">Get more local customers (Google Maps)</SelectItem>
                  ...
                  <SelectItem value="seo-friendly-website">Build a SEO Friendly Website</SelectItem>
            ...
            <div className="space-y-2">
              <Label className="text-sm font-medium">Monthly Budget Range* <span className="text-red-500">*</span></Label>
              <Select
                value={formData.budgetRange}
                onValueChange={(value) => updateFormData({ budgetRange: value })}
              >
                <SelectTrigger className={`input bg-[#F9FAFB] w-full ${errors.budgetRange ? 'border-red-500' : ''}`}>
            ...
            <div className="space-y-2 ">
              <label className="flex items-start text-sm my-auto gap-2 p-5 rounded-[12px] bg-[#F9FAFB]  text-[#5B6475]">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => updateFormData({ privacyAgreement: checked as boolean })}
                  className="border border-[#D1D5DB]"
                />
                <span>
                  I agree to the <b>Privacy Policy</b> and consent to be contacted
                  by Md Faruk Khan regarding my inquiry. <span className="text-red-500">*</span>
                </span>
              </label>
              {errors.privacyAgreement && <p className="text-red-500 text-xs">{errors.privacyAgreement}</p>}
            </div>
        ...
        <div className="py-6 px-6 text-center">
          <p className="text-xs font-medium text-[#5B6475] mb-3">
            Trusted by Brands
          </p>
          <div className="flex flex-wrap justify-center gap-3">
            {['104% Organic Traffic', 'Top Furniture Rankings', 'YouTube SEO Success', 'Complete Digital SEO', 'Brand Authority Building'].map(
              brand => (
                <div
                  key={brand}
                  className="px-3 py-2 bg-[#F1F5F9] rounded-md text-xs font-medium text-[#0B1220]"
                >
                  {brand}
                </div>
              )
